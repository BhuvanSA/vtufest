...
export default function GetRegistrant({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const toggleAttendance = async(usn:string,registrantId: string, eventId: string) => {

    console.log(eventId,usn,registrantId)
    const updatedRegistrations = studentData?.eventRegistrations.map((registration) =>
      registration.registrantId === registrantId && registration.eventId === eventId
        ? { ...registration, attendanceStatus: !registration.attendanceStatus }
        : registration
    );

    if (studentData) {
      setStudentData({ ...studentData, eventRegistrations: updatedRegistrations! });
    }
      ...
      <div className="w-full max-w-6xl bg-white shadow-md rounded-lg p-6 pt-24">
        ...
        <div className="bg-white shadow-md rounded-lg p-4 mb-6">
          <div className="flex items-center space-x-6">
            ...
            <div>
              <div
                className={`text-xl font-semibold ${isVerified ? "text-green-500" : "text-red-500"}`}
              >
                {studentData?.name}
                <button
              ...
              <div className="text-blue-600 font-medium">{studentData?.type}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="bg-white shadow-md rounded-lg p-4">
            <div className="text-lg font-semibold mb-4">Events Registered</div>
            <ul className="space-y-2">
              {studentData?.eventRegistrations.map((registration) => {
                const event = studentData?.events.find((e) => e.id === registration.eventId);
                ...
                return (
                  <li
                    ...
                    className={`p-3 rounded-lg ${
                      registration.attendanceStatus ? "bg-blue-50" : "bg-gray-50"
                    } flex justify-between items-center`}
                  >
                    <span>{event.eventName}</span>
                    <div className="flex space-x-2 items-center">
                      <button
                        onClick={() => toggleAttendance(studentData.usn,registration.registrantId, registration.eventId)}
                        className={`${
                          registration.attendanceStatus ? "bg-green-500" : "bg-red-500"
                        } text-white px-4 py-2 rounded`}
                      >
                        {registration.attendanceStatus ? "Attended" : "Mark as Attended"}
                      </button>
                    </div>
                  </li>
                );

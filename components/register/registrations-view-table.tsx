...
export function DataTable({ data }: { data: Data[] }) {
    ...
    const columns = React.useMemo<ColumnDef<Data>[]>(
        () => [
            ...
            },
            {
                ...
                header: ({ column }) => {
                    const filterCycle = ["ALL", "Bengaluru Region (1)", "Belgavi Region (2)", "Kalaburgi Region (3)", "Mysuru Region (4)"];
                    const currentFilter =
                        (column.getFilterValue() as string) ?? "ALL";
                    const currentIndex = filterCycle.indexOf(currentFilter);
                    const nextIndex = (currentIndex + 1) % filterCycle.length;
    ...
    const handleExportToPDF = () => {
        ...
        const headers = [
            ["Name", "USN", "College Name", "Type", "College Code", "College Region", "Email", "Phone", "Accomodation", "Date Of Birth", "Gender", "Designation", "Events"],
        ];
